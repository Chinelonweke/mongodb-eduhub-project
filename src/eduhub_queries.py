...
# 1. Find courses priced between $50 and $20
...
# 2. Get users who joined in the last 6 month
...
# 3. Find courses with specific tags using $i
...
# 4. Retrieve assignments due in the next 7 day
today = datetime.now()
...
# 5. Count total enrollments per cours
pipeline = [
...
# 11. Instructor analysis – total students taugh
pipeline = [
    {"$lookup": {
        "from": "courses",
...
# 12. Average course rating per instructo
pipeline = [
...
# 13. Revenue generated per instructo
pipeline = [
    {"$lookup": {
        "from": "courses",
        "localField": "course_id",
...
# 15. Most popular course categorie
pipeline = [
    {"$lookup": {
        "from": "courses",
        "localField": "course_id",
